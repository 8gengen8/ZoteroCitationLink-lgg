...
'Determine whether the bookmark was generated by ZoteroLinkCitation.
Private Function IsZoteroBookmark(bmName As String) As Boolean
    'Rules: Bookmarks generated by ConvertToBookmarkName
    ' 1. Length ≤ 40 (or within 44)
    ' 2. Contains only 0-9, A-Z, a-z, and underscores
    ' 3. May start with an underscore or a letter
    Dim re As Object
    Set re = CreateObject("VBScript.RegExp")
    With re
        .Pattern = "^[A-Za-z_][A-Za-z0-9_]{0,43}$"
        .IgnoreCase = True
    End With
    IsZoteroBookmark = re.Test(bmName)
End Function
...
Private Sub ZoteroLinkCitationWithinSelection()
    'Add Zotero citation hyperlinks,one by one
...
End Sub
'-------------------------------------------------------------------
'Function of ZoteroLinkCitation hyperlinks
'-------------------------------------------------------------------
Private Sub FuncZoteroUnlinkCitations()
    ...
    '1.Traverse all hyperlinks.
    Dim hp As Hyperlink
    
    'Must delete from back to front to avoid index misalignment.
    Dim i As Long
    For i = doc.Hyperlinks.Count To 1 Step -1
        Set hp = doc.Hyperlinks(i)
        'Only process internal links where SubAddress is not empty.
        If hp.SubAddress <> "" Then
            'Is ZoteroBookmark？
            If IsZoteroBookmark(hp.SubAddress) Then
                'hp.Range.Font.ColorIndex = wdAuto 'Restore the original black quote:wdAuto→black,wdRed→red、wdGreen→green、wdBlue→blue
                hp.Delete 'Delete hyperlink and keep the text.
            End If
        End If
    Next i
    
    '2.Delete all ZoteroBookmark
    Dim bk As Bookmark
    For Each bk In doc.Bookmarks
        If IsZoteroBookmark(bk.name) Then
            bk.Delete
        End If
    Next bk
    Application.ScreenUpdating = True
End Sub
'Delete ZoteroLinkCitation hyperlinks
Public Sub ZoteroUnlinkCitations()
    Call FuncZoteroUnlinkCitations 'Delete hyperlink
    Call SetZoteroCitationColor 'Add citation color
    MsgBox "Remove ZoteroCitations Successfully!", vbInformation
End Sub

'Automatically add all links.
Public Sub ZoteroLinkCitationAll()
    'Delete Zotero citation hyperlinks
    Call FuncZoteroUnlinkCitations
    'Add Zotero citation hyperlinks
    Dim originalRng As Range
    Set originalRng = Selection.Range

    Dim debugging As Boolean
    debugging = False 'Select Automatically add(False) or Manually add(True)
    ...
    Exit Sub
End Sub
'Manually add citation links one by one.
Public Sub ZoteroLinkCitationSelect()
    'Delete Zotero citation hyperlinks
    Call FuncZoteroUnlinkCitations
    'Add Zotero citation hyperlinks
    Dim originalRng As Range
    ...
    Dim debugging As Boolean
    debugging = True 'Select Automatically add(False) or Manually add(True)
    ...
    Exit Sub
End Sub
'Set Zotero Citation Color
Private Sub SetZoteroCitationColor()
    Dim fld As field
    Dim jsonObj As Object
    Application.ScreenUpdating = False
    ' Traverse all fields in the document.
    For Each fld In ActiveDocument.Fields
        ' Check if it is a Zotero citation field.
        If fld.Type = wdFieldAddin Then
            If InStr(1, fld.code.Text, "ADDIN ZOTERO_ITEM", vbTextCompare) > 0 Then
                ' Analyze the specific citation items in the reference section.
                Set jsonObj = ParseCSLCitationJson(fld.code.Text)
                ' Directly set the color of the entire citation field.
                fld.result.Font.ColorIndex = wdAuto 'Set Zotero Citation Color:wdAuto→black,wdRed→red、wdGreen→green、wdBlue→blue
            End If
        End If
    Next fld
    Application.ScreenUpdating = True
...
Private Sub ZoteroLinkCitation(targetFields, Optional debugging As Boolean = False, Optional notify As Boolean = False) 'If manually select style, tnotify set True
            ...
            If InStr(aField.code, "ADDIN ZOTERO_ITEM") > 0 Then
                ...
                For i = 0 To UBound(cits)
                    ...
                    Set hp = ActiveDocument.Hyperlinks.Add( _
                        ...
                        SubAddress:=titleAnchor, ScreenTip:=title) 'ScreenTip set displayed information of hyperlink
                    hp.Range.Font.Underline = wdUnderlineNone 'Remove hyperlink underline
                    'hp.Range.Font.ColorIndex = wdAuto 'Set hyperlink color:wdAuto→black,wdRed→red、wdGreen→green、wdBlue→blue
...
'支持的参考文献样式
Public Sub SupportStyleInformation()
    Dim styles As Variant
    styles = Split("olecular-plant|ieee|apa|vancouver|american-chemical-society|erican-medical-association|nature|" & _
                   "american-political-science-association|rican-sociological-association|chicago-author-date|bmc-medicine|" & _
    ...
    Next
    MsgBox msg, vbInformation, "支持的参考文献样式"
End Sub
